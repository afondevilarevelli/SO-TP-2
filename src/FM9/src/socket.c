...
// Cantidad de conexiones pendientes  que se mantienen en cola
#define BACKLOG 10
...
void receiptMessage(void * arguments)
{
	...
	// bytes recibidos por cada lectura (0 = desconexion del cliente)
	int numbytes;
	// buffer de lectura
	char buffer[MAXDATASIZE];
	// determina si se analizo por lo menos una vez un nuevo set de datos
	bool bufferUsed;

	// cantidad de argumentos del packete que recibo
	unsigned int n = -1;
	// ayuda a saber la ultima posicion de lectura en el buffer (tambien se usa como i en otros casos)
	int i = 0;
	// j de for
	int j = 0;
	// cantidad de separadores ,(coma) contados en el encabezado del paquete
	int seps = 0;
	// encabezado del paquete
	char * packet_head;
	// cuerpo del paquete
	char * packet_body;
	// datos excedidos del final del paquete
	char * exceeded;
	// temporar para operaciones de string
	char * str_tmp;
	// determina si hay datos excedidos cargados en el buffer
	bool isExceeded = false;

// determina si ya fue recibido el encabezado del paquete
	bool fullHead = false;
	// determina los bytes que contendrá el cuerpo del paquete
	int bytesBody = 0;
	// encabezado divido por n, name y bytes de los args
	char ** head = NULL;
	// utilizado para extraer bytes del buffer
	char b[2];
	// utilizado para guardar la funcion que hay que ejecutar del cliente
	void (*fn)();
	// Argumentos que se envian por la funcion
	char ** packet_args;
	...
	while (1) {
		...
		// analizo el buffer si es la primera vez que lo voy a analizar o si
		// queda informacion excedida para seguir analizando
		...
		while (!bufferUsed || isExceeded) {
			...
			if (!fullHead) {
				for (i = 0; i < strlen(buffer); i++)	{
					...
					//finalizo cuando llego al ultimo separador
					if (n+2 == seps) {
						fullHead = true;
			...
			if (fullHead) {
				...
				if (bytesBody <= strlen(packet_body)) {
					// si el paquete esta excedido, corto el paquete en la posicion
					// correspondiente y guardo el excedente sobre el buffer
					if (bytesBody < strlen(packet_body)) {
						isExceeded = true;
					...
					if (args->fns_receipts != NULL) {
						...
						// Si n=0 el split no toma el vacio despues de la coma final.. quito
						// ultima coma
						if (n==0) {
...
bool runFunction(int socket, char * name, int n, ...) {
	...
	// Cabeza del packete, lo inicio con la cantidad de argumentos que tendra
	char * packet_head = string_itoa(n);
	// Continuo con coma
	string_append(&packet_head, ",");
	// Continuo con el nombre de la funcion
	string_append(&packet_head, name);
	// Continuo con coma
	string_append(&packet_head, ",");

	// Cuerpo del packete
	char * packet_boody = string_new();
	...
	for (i = 0; i < n; i++)	{
		...
		if(arg == NULL) {
			printf("[%s] Error, no se encuentran los parametos indicados", name);
			exit(1);
		}

		// Agrego las dimensiones de cada argumento y las separo por comas (en el cabezal)
...
void listenClients(void * arguments) {
	...
	// Escuchar sobre socket_client, nuevas conexiones sobre newConnection
	int newConnection;
	// hilo para crear receptor de mensajes
	pthread_t th_receiptMessage;
...
int createListen(int port, void (*fn_newClient)(), t_dictionary * fns, void (*fn_connectionClosed)(), void * data) {
	...
	// Escuchar sobre socket_client, nuevas conexiones sobre newConnection
	int socket_client;
	// información sobre la direccion del servidor
	...
	// Configuración de dirección de servidor
	// Ordenación de bytes de la máquina (Siempre va AF_INET)
	addr_server.sin_family = AF_INET;
	// short, Ordenación de bytes de la red
	addr_server.sin_port = htons(port);
